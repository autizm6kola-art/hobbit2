...
// const APP_ID = "hobbit2"; // 👈 уникальное имя книги
...
export default function ReadingTask({ task }) {
  ...
  const handleResult = useCallback((transcript) => {
    ...
    const transcriptWords = normalizeToArray(transcript);
    ...
    if (activeWordIndex !== null) {
      ...
      const targetWord = content[activeWordIndex]?.word
        ?.toLowerCase()
        .replace(/[.,!?;:«»"()\r\n]/g, "");
    ...
    const availableTokens = [...transcriptWords];
    ...
    content.forEach((item, index) => {
      ...
      const foundIndex = availableTokens.findIndex(
        tok => tok === clean
      );
    ...
    const trulyNew = merged.filter(
      index => !highlightedIndexes.includes(index)
    );
